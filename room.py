...
class Room():
    ...
    def get_long_description(self):
        ...
        if len(self.characters) == 0:
            ...
            if len(self.items) == 0 and len(self.characters) == 1:
            
                return "You are " + self.description +", "+self.characters[0].name.title()+" is in this area"+ ".\n" + self.get_exit_string()
        
            elif len(self.items) == 1 and len(self.characters) == 1:
                ...
                for i in range(len(self.characters)):
                    if self.characters[i] == self.characters[-1]:
                        room_defination += self.characters[i].name.title() + " are in this area.\nThere is "
                        
                    elif len(self.items) == 1:
                        room_defination +=  self.characters[i].name.title() + " and "
                        
                    else :
                        room_defination += self.characters[i].name.title() + ", "
                        
                for i in range(len(self.items)):
                    if self.items[i] == self.items[-1]:
                        room_defination += "a " + Item.get_item_info(self.items[i])
                    elif len(self.items) == 1:
                        room_defination += "a " + Item.get_item_info(self.items[i]) + " and "
                    else :
                        room_defination += "a " + Item.get_item_info(self.items[i]) + ", "
                
                return first_string + ", " + room_defination + ".\n" + self.get_exit_string()
